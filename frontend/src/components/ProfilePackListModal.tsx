...
export default function ProfilePackListmodal({
  ...
  console.log(selectedPackList?._id);
  ...
  const handleSubmit = () => {
    ...
    if(addNew){
      ...
      if (!selectedPackList || !selectedPackList._id) {
        console.error("Selected pack list is missing or has no ID", selectedPackList);
        return;
      }
  
      const userPackListId = selectedPackList._id; // Ensure this is defined
      const updatedPackList = { name, items };
      
      updateUserPackList(userId, userPackListId, token, userData, setUserData, updatedPackList);
    }
